const features = [
  {
    ...
    description: 'Keep projects, notes and tasks connected inside focused collaborative workspaces.',
    icon: `
  ...
  },
  {
    ...
    description: 'Track priorities, progress, and active work with workflows built for smaller focused teams.',
    icon: `
      <svg viewBox="0 0 24 24" aria-hidden="true">
        <path d="M9.25 7.75h8" />
        <path d="M9.25 12h8" />
  ...
  {
    title: 'Kanban / Table / Calendar views',
    description:
      'Switch between visual planning, structured tables, and calendar scheduling depending on how your team works.',
    icon: `
      <svg viewBox="0 0 24 24" aria-hidden="true">
  ...
  },
  {
    ...
    description: 'Organize members, responsibilities, and project ownership inside one shared operational workspace.',
    icon: `
      <svg viewBox="0 0 24 24" aria-hidden="true">
...
const progressItems = [
  ...
  'Core task workflows evolving',
  'UI system refinement ongoing',
  'Notes and documentation structure expanding',
  'Product architecture continuously improving',
]

const devlogEntries = [
  {
    title: 'Redesigned workspace shell',
    detail: 'Refining the outer workspace structure to feel calmer, clearer, and easier to navigate.',
    icon: `
      <svg viewBox="0 0 24 24" aria-hidden="true">
        <rect x="4.25" y="5.25" width="15.5" height="13.5" rx="2.25" />
  ...
  {
    title: 'Project and task organization',
    detail: 'Tightening the relationship between projects, tasks, and daily organization flows.',
    icon: `
      <svg viewBox="0 0 24 24" aria-hidden="true">
  ...
  {
    title: 'Kanban, Table, and Calendar workflows',
    detail: 'Shaping the core task experience across multiple views without losing cohesion.',
    icon: `
      <svg viewBox="0 0 24 24" aria-hidden="true">
        <rect x="4.25" y="5.25" width="15.5" height="13.5" rx="2.25" />
        <path d="M9.25 5.25v13.5" />
  ...
  {
    title: 'Full-stack platform foundation',
    detail: 'Building the platform from scratch across frontend, backend/API, and PostgreSQL.',
    icon: `
      <svg viewBox="0 0 24 24" aria-hidden="true">
...
const footerLinks = [
  {
    label: 'LinkedIn',
    href: 'https://www.linkedin.com/in/philip-agn%C3%A9-a86679411/',
    external: true,
    icon: `
      <svg viewBox="0 0 24 24" aria-hidden="true">
  ...
  {
    label: 'GitHub',
    href: 'https://github.com/philipvagne',
    external: true,
    icon: `
      <svg viewBox="0 0 24 24" aria-hidden="true">
...
const summaryItems = ['Projects', 'Tasks', 'Notes', 'Team workflows', 'Shared visibility']

const buildSnapshotItems = [
  {
    label: 'Frontend',
    detail: 'React workspace interface',
  },
  {
    label: 'Backend',
  ...
  {
    label: 'Product',
    detail: 'Projects, tasks, notes, and teams',
  },
]

const productDirectionItems = [
  {
    title: 'Plan work',
    description: 'Create focused spaces for projects, notes, and shared planning.',
    icon: `
      <svg viewBox="0 0 24 24" aria-hidden="true">
        <path d="M4.75 6.75h14.5" />
        <path d="M4.75 12h9.5" />
  ...
  },
  {
    ...
    description: 'Move tasks through Kanban, table, and calendar workflows.',
    icon: `
      <svg viewBox="0 0 24 24" aria-hidden="true">
        <rect x="4.25" y="5.25" width="15.5" height="13.5" rx="2.25" />
        <path d="M9.25 5.25v13.5" />
  ...
  },
  {
    ...
    description: 'Keep members, ownership, activity, and updates visible in one workspace.',
    icon: `
      <svg viewBox="0 0 24 24" aria-hidden="true">
    ...
    .map(
      (area) => `
        <article class="status-card">
          <div class="status-icon">${area.icon}</div>
          <p>${area.title}</p>
        </article>
      `,
    )
...
const renderBuildSnapshotItems = () =>
  buildSnapshotItems
    .map(
      (item) => `
...
const renderProductDirectionItems = () =>
  productDirectionItems
    .map(
      (item) => `
        <article class="direction-row">
          <div class="direction-icon">${item.icon}</div>
...
export function setupApp() {
  const contactTrigger = document.querySelector('[data-action="contact-modal"]')
  const modal = document.querySelector('[data-contact-modal]')
  const modalBackdrop = document.querySelector('[data-contact-backdrop]')
  const modalClose = document.querySelector('[data-contact-close]')
  const form = document.querySelector('[data-contact-form]')
  const success = document.querySelector('[data-contact-success]')

  if (!modal || !contactTrigger) {
    return
  }
  ...
  form?.addEventListener('submit', (event) => {
    ...
    const subject = encodeURIComponent(`Zorune contact from ${name || 'site visitor'}`)
    const body = encodeURIComponent(`Name: ${name}\nEmail: ${email}\n\n${message}`)

    success?.removeAttribute('hidden')
    window.location.href = `mailto:Philipv.agne@gmail.com?subject=${subject}&body=${body}`
  })
}
